...
void get_var_ids(int ncid, const char *var_names[NUM_VARS], int var_ids[],
                 int count) {
  /**
   * @param ncid id of netcdf file
   * @param var_names array of netcdf variable names for which ids should be
   * extracted
...
int read_velocity(int ncid, const char *options[NUM_VARS], const char *var_name,
                  const int *start_idxs, const int *count_idxs,
                  const int *stride_steps,
                  float target_buffer[TIME][DEPTH][NODE2]) {
  /**
   ...
   * @param start_idxs : array of ints representing the starting point (each
   * dimension) for reading a variable in ncid
   * @param count_idxs : array of ints representing the end point (each
   * dimension) for reading a variable in ncid
   * @param stride_step : array of ints representing the step with which data is
   * read
   * @returns error status (status_buffer), if no errors occur, const int
   * NC_NOERR is returned
   */
  ...
  const ptrdiff_t stridev[3] = {stride_steps[0], stride_steps[1],
                                stride_steps[2]};
  ...
  if (!flag) {
    ...
    if (status_varid == NC_NOERR) {
      status_buffer = nc_get_vars_float(ncid, var_id, &startv[0], &countv[0],
                                        &stridev[0], &target_buffer[0][0][0]);
...
int check_status(int *status, const char *var_name) {
   ...
   * @param var_name : name of the variable read in a command that returns
   * status
   */
  int local_status = 0;
  if ((*status) != NC_NOERR) {
...
float **compute_maximum(const int var_idx, float (*matrix)[DEPTH][NODE2],
                        int *dimension) {
  /**
...
void compute_norm(float vnode_mat[TIME][DEPTH][NODE2],
                  float unode_mat[TIME][DEPTH][NODE2],
                  float target_matrix[TIME][DEPTH][NODE2]) {
  for (int time = 0; time < TIME; time++) {
    for (int depth = 0; depth < DEPTH; depth++) {
      for (int node = 0; node < NODE2; node++) {
        target_matrix[time][depth][node] =
            sqrt((pow(vnode_mat[time][depth][node], 2) +
                  pow(unode_mat[time][depth][node], 2)));
      }
    }
  }
}

void compute_inverted_max(int dim1, int dim2, int dim3,
                          float local_velocity[dim3][dim1][dim2],
                          float local_max[dim3][dim1]) {
  /**
   * @brief Compute max along the second dimension (dim2) passed as parameter.
   * It is and adaptation of the computation of max, since data, after
   * gathering, comes in a different shape. Stores result in local_max.
   */
